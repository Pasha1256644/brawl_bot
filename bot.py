...
async def handle_balance_topup(message: Message, state: FSMContext):
    if is_admin(message.from_user.id):
        ...
        await message.answer("💳 <b>Пополнение баланса</b>\n\nОбратитесь в поддержку: @suport_skup_bs_bot", reply_markup=profile_kb, parse_mode="HTML")
...
async def earn_yes(callback: CallbackQuery, state: FSMContext):
    ...
    if is_worker(user_id):
        ...
        await callback.message.answer(
            ...
            "4️⃣ Начните писать комментарии в TikTok\n"
            "5️⃣ Отправляйте скриншоты в этот чат\n"
            "6️⃣ Получите награду после выполнения!\n\n"
...
# ============================================
# ===== РАБОТНИК (С ПОДДЕРЖКОЙ АЛЬБОМОВ И ШТРАФАМИ) =====
# ============================================
...
async def handle_work_start(message: Message, state: FSMContext):
    ...
    # Нет активного задания - показываем инструкцию
    await message.answer(
...
async def process_worker_photo(message: Message, state: FSMContext, count: int = 1):
    ...
    # Проверка на повторную отправку скрина
    if photo_hash in task["file_hashes"]:
        # ШТРАФ: списание награды И заморозка на 48 часов
        reward = task["reward"]
        users_db[user_id]["balance"] -= reward
        add_history(user_id, -reward, f"Штраф за повторную отправку скрина (-{reward} ⭐)")
        
        # Заморозка на 48 часов
...
async def moderator_msg_user_input(message: Message, state: FSMContext):
    ...
    if len(parts) < 2:
        await message.answer("❌ Введите ID и текст через пробел.", reply_markup=back_kb, parse_mode="HTML")
        return
    ...
    try:
        ...
        await message.answer(f"✅ Сообщение отправлено!", reply_markup=moderator_menu_kb)
