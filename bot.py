...
async def clean_old_tasks():
    ...
    for user_id, task in user_tasks.items():
        if (now - task["start_time"]).total_seconds() > 48 * 3600:
...
async def cmd_start(message: Message, state: FSMContext):
    ...
    if message.from_user.id == OWNER_USERNAME:
        await message.answer("👋 <b>Добро пожаловать, владелец!</b>\n\n❓ <b>Что вы хотите сделать?</b>\n\n🔹 У вас есть доступ к админ-панели (кнопка ниже).", reply_markup=owner_menu_kb, parse_mode="HTML")
    elif message.from_user.id in WORKER_IDS:
        await message.answer("👋 <b>Добро пожаловать!</b>\n\n❓ <b>Что вы хотите сделать?</b>", reply_markup=worker_menu_kb, parse_mode="HTML")
    else:
        await message.answer("👋 <b>Добро пожаловать!</b>\n\n❓ <b>Что вы хотите сделать?</b>", reply_markup=main_menu_kb, parse_mode="HTML")
...
async def handle_donate(message: Message, state: FSMContext):
    ...
    for item in DONATE_ITEMS:
        keyboard.inline_keyboard.append([InlineKeyboardButton(text=f"🛒 Купить {item['name']} за {item['price']} ⭐", callback_data=f"donate_buy_{item['name'].lower().replace('+', 'p')}")])
...
async def handle_sell(message: Message, state: FSMContext):
    ...
    await message.answer("💰 <b>Продажа аккаунта</b>\n\n👇 <b>Выберите действие:</b>", reply_markup=sell_menu_kb, parse_mode="HTML")
...
async def handle_list_item(message: Message, state: FSMContext):
    ...
    await message.answer("📦 <b>Выставление товара</b>\n\n📩 Чтобы выставить товар, отправьте своё объявление сюда:\n👤 <b>@suport_skup_bs_bot</b>\nили воспользуйтесь кнопкой «Поддержка» в главном меню бота.\n\n📋 <b>По шаблону:</b>\n━━━━━━━━━━━━━━━━━━━━━\n📸 Фото аккаунта\n🏆 Количество кубков\n💎 Количество гемов\n⚔️ Количество бойцов\n💰 Цена\n━━━━━━━━━━━━━━━━━━━━━\n\n📌 <b>Пример:</b>\n📸 [фото]\n🏆 25000 кубков\n💎 500 гемов\n⚔️ 45 бойцов\n💰 1000 ⭐\n\n✅ После проверки модератором, ваше объявление появится в боте.", reply_markup=back_to_main_kb, parse_mode="HTML")
...
async def admin_assign_status_input(message: Message, state: FSMContext):
    ...
    if not status:
        await message.answer("❌ Неверный статус. Доступные статусы: Админ, Поддержка, Тестер, VIP, Работник, Пользователь.", reply_markup=back_to_main_kb, parse_mode="HTML")
        return
    data = await state.get_data()
    ...
    await message.answer(f"✅ <b>Статус пользователя изменён!</b>\n\n👤 Пользователь: {user_data['name']} ({user_data['username']})\n🎖️ Новый статус: {emoji} {new_status}", reply_markup=admin_menu_kb, parse_mode="HTML")
    await state.set_state(ScamStates.admin_panel)
...
async def admin_balance_amount_input(message: Message, state: FSMContext):
    ...
    if amount > 0:
        admin_text = f"💰 <b>ПОПОЛНЕНИЕ БАЛАНСА</b>\n👤 Пользователь: {users_db[target_user]['username']} (ID: {target_user})\n📛 Имя: {users_db[target_user]['name']}\n➕ Сумма: +{amount} ⭐\n📊 Новый баланс: {new_balance} ⭐"
    else:
        admin_text = f"💰 <b>СПИСАНИЕ БАЛАНСА</b>\n👤 Пользователь: {users_db[target_user]['username']} (ID: {target_user})\n📛 Имя: {users_db[target_user]['name']}\n➖ Сумма: {amount} ⭐\n📊 Новый баланс: {new_balance} ⭐"
...
async def admin_freeze_user_input(message: Message, state: FSMContext):
    ...
    user_data = users_db[target_user]
    await message.answer(f"✅ Найден пользователь: {user_data['name']} ({user_data['username']})\nТекущий статус заморозки: {'активен' if user_data['frozen_until'] is None else 'заморожен до ' + (user_data['frozen_until'].strftime('%d.%m.%Y %H:%M') if isinstance(user_data['frozen_until'], datetime) else 'навсегда')}\n\nВведите <b>дату окончания заморозки</b> в формате:\n<b>ДД.ММ.ГГГГ ЧЧ:ММ</b> (по московскому времени)\nили <b>0</b> для бессрочной заморозки.\n\n🔙 Для отмены нажмите 'Назад'.", reply_markup=back_to_main_kb, parse_mode="HTML")
...
async def main():
    print("🤖 Бот запущен (полная версия, оптимизированная)")
