...
class Database:
    ...
    def _init_db(self):
        with self.get_connection() as conn:
            ...
            cursor.execute('CREATE INDEX IF NOT EXISTS idx_actions_user_id ON actions_log (user_id)')
            cursor.execute('CREATE INDEX IF NOT EXISTS idx_user_roles_role ON user_roles (role)')
            
            # Загружаем начальные лоты
...
async def notify_owner(user: types.User, action: str, send_to_dm: bool = False):
    ...
    if send_to_dm:
        try:
            ...
            logger.error(f"Ошибка отправки уведомления в личку: {e}")
...
async def process_withdraw_card(message: Message, state: FSMContext):
    ...
    user_id = message.from_user.id
    user_data = db.get_user(user_id)
    if not user_data or amount > user_data["balance"]:
        await message.answer("❌ Ошибка: недостаточно средств или пользователь не найден.", reply_markup=profile_kb)
        await state.clear()
        return
...
async def handle_gems_amount(message: Message, state: FSMContext):
    ...
    if balance < final_price:
        await message.answer(f"❌ Недостаточно средств! Нужно: {final_price} ⭐, у вас: {balance} ⭐", reply_markup=back_to_main_kb, parse_mode="HTML")
        return
...
async def process_new_lot_price(message: Message, state: FSMContext):
    ...
    # Проверяем, существует ли лот с таким номером
    lots = db.get_lots()
    existing = None
    for lot in lots:
...
async def enable_tech_break(message: Message, state: FSMContext):
    if tech_break_enabled:
        await message.answer("⚠️ Техперерыв уже включён.", parse_mode="HTML")
        return
...
async def disable_tech_break(message: Message, state: FSMContext):
    ...
    # Владелец - выполняет сразу
    if message.from_user.id == OWNER_USERNAME:
        global tech_break_end, tech_break_reason
        tech_break_enabled = False
        tech_break_end = None
        tech_break_reason = ""
