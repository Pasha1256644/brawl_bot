...
# ===== ГЛОБАЛЬНЫЕ ПЕРЕМЕННЫЕ ТЕХПЕРЕРЫВА =====
...
async def show_history(message: Message, state: FSMContext):
    ...
    if not history:
        await message.answer(
            ...
            "Здесь будут отображаться все пополнения и траты.",
            reply_markup=profile_kb, parse_mode="HTML"
        )
        return
    ...
    for entry in entries:
        ...
        text += f"🕒 {entry['time']}\n"
        text += f"   {sign}{entry['amount']} ⭐ — {entry['description']}\n\n"
...
async def handle_balance_topup(message: Message, state: FSMContext):
    ...
    if message.from_user.id == OWNER_USERNAME:
        await message.answer(
            "👑 Вы владелец. Используйте админ-панель для пополнения баланса пользователей.",
            reply_markup=profile_kb, parse_mode="HTML"
        )
    else:
        await message.answer(
...
async def start_withdraw(message: Message, state: FSMContext):
    ...
    if frozen:
        if until == "навсегда":
            ...
            until_str = until.strftime("%d.%m.%Y %H:%M") + " (МСК)"
        await message.answer(
            f"❄️ <b>Ваш профиль заморожен!</b>\n\n"
            f"⏰ До: {until_str}\n"
            f"📌 Причина: {reason}\n\n"
            f"Данная функция вам недоступна.\n"
            f"Если вы не согласны с решением, обратитесь в поддержку: @suport_skup_bs_bot\n"
            f"или воспользуйтесь кнопкой «Поддержка» в главном меню бота.",
            reply_markup=profile_kb, parse_mode="HTML"
        )
        return
...
async def process_withdraw_card(message: Message, state: FSMContext):
    ...
    if amount > user_data["balance"]:
        await message.answer(
            ...
            "Начните процесс вывода заново через профиль.",
            reply_markup=profile_kb, parse_mode="HTML"
        )
        await state.clear()
        return
    ...
    await message.answer(
        f"✅ <b>Отлично!</b>\n\n"
        f"Средства в размере <b>{amount} ⭐</b> будут зачислены на указанную карту в течение <b>2-3 недель</b>.\n"
        f"Комиссия за вывод составит <b>5%</b> (это <b>{commission:.2f} ⭐</b>).\n\n"
    ...
    for item in DONATE_ITEMS:
        keyboard.inline_keyboard.append([
            InlineKeyboardButton(
                ...
                callback_data=f"donate_buy_{item['name'].lower().replace('+', 'p')}"
            )
        ])
    keyboard.inline_keyboard.append([
...
async def process_donate_buy(callback: CallbackQuery, state: FSMContext):
    ...
    if balance < price:
        await callback.answer(
            "❌ Недостаточно средств! Пополните баланс через поддержку: @suport_skup_bs_bot\n"
            "или воспользуйтесь кнопкой «Поддержка» в главном меню бота.",
            show_alert=True
        )
        await callback.message.answer(
            f"❌ <b>Недостаточно средств!</b>\n\n"
            f"Для покупки <b>{target_item['name']}</b> требуется <b>{price} ⭐</b>, а у вас <b>{balance} ⭐</b>.\n"
        ...
        await callback.answer()
        return
    users_db[user.id]["balance"] -= price
    new_balance = users_db[user.id]["balance"]
    add_history_entry(user.id, -price, f"Покупка {target_item['name']}")
    admin_text = (
        f"⭐ <b>ПОКУПКА ДОНАТА</b>\n"
        f"👤 Пользователь: {user_data['username']} (ID: {user.id})\n"
        f"📛 Имя: {user_data['name']}\n"
        f"🎯 Купил: {target_item['name']}\n"
        f"💰 Сумма: {price} ⭐\n"
        f"📊 Остаток: {new_balance} ⭐"
    )
    ...
    await callback.message.answer(
        f"✅ <b>Отлично!</b>\n\n"
        f"Вы успешно приобрели <b>{target_item['name']}</b> за {price} ⭐.\n"
        f"📊 Новый баланс: {new_balance} ⭐\n\n"
        f"Скоро с вами свяжется наш оператор.\n"
...
async def handle_gems_amount(message: Message, state: FSMContext):
    ...
    try:
        ...
        await message.answer(
            "❌ <b>Ошибка!</b>\n\n"
            "Пожалуйста, введите <b>положительное число</b> (например, 10, 50).",
            reply_markup=back_to_main_kb, parse_mode="HTML"
        )
        return
    ...
    if balance < stars:
        await message.answer(
            f"❌ <b>Недостаточно средств!</b>\n\n"
            f"Для покупки <b>{gems_count} гемов</b> требуется <b>{stars} ⭐</b>, а у вас <b>{balance} ⭐</b>.\n"
            "Пополните баланс через поддержку: @suport_skup_bs_bot\n"
            "или воспользуйтесь кнопкой «Поддержка» в главном меню бота.",
            reply_markup=back_to_main_kb, parse_mode="HTML"
        )
    ...
    new_balance = users_db[user_id]["balance"]
    add_history_entry(user_id, -stars, f"Покупка гемов {gems_count}шт")
    admin_text = (
        f"💎 <b>ПОКУПКА ГЕМОВ</b>\n"
        f"👤 Пользователь: {user_data['username']} (ID: {user_id})\n"
        f"📛 Имя: {user_data['name']}\n"
        f"💎 Купил: {gems_count} гемов\n"
        f"💰 Сумма: {stars} ⭐\n"
        f"📊 Остаток: {new_balance} ⭐"
    )
    ...
    await message.answer(
        f"✅ <b>Отлично!</b>\n\n"
        f"Вы успешно приобрели <b>{gems_count} гемов</b> за {stars} ⭐.\n"
        f"📊 Новый баланс: {new_balance} ⭐\n\n"
        f"Скоро с вами свяжется наш оператор.\n"
        f"Спасибо за покупку! 🎉\n"
        f"По всем вопросам обращайтесь в поддержку: @suport_skup_bs_bot\n"
...
async def handle_account_buy(message: Message, state: FSMContext):
    ...
    text += "⚠️ <b>Обязательно прочтите информацию в разделе \"Как проходит сделка?\"</b>\n\n"
    keyboard = InlineKeyboardMarkup(inline_keyboard=[])
    for idx, lot in enumerate(LOTS, start=1):
        text += (
            f"━━━━━━━━━━━━━━━━━━━━━\n"
            f"🎯 <b>{lot['name']}</b>\n"
            f"🏆 <b>{lot['cups']} кубков</b>\n"
            f"💎 <b>{lot['gems']} гемов</b>\n"
            f"⚔️ <b>{lot['fighters']} бойцов</b>\n"
            f"💰 <b>Цена: {lot['price']} ⭐</b>\n"
            f"📩 <i>по всем вопросам обращайтесь в поддержку @suport_skup_bs_bot\n"
            f"или воспользуйтесь кнопкой «Поддержка» в главном меню бота.</i>\n"
        )
        keyboard.inline_keyboard.append([
            InlineKeyboardButton(text=f"🛒 Купить {lot['name']}", callback_data=f"buy_lot_{idx}")
        ])
    text += "━━━━━━━━━━━━━━━━━━━━━\n"
...
async def process_buy_lot(callback: CallbackQuery, state: FSMContext):
    ...
    if balance < price:
        await callback.answer(
            "❌ Недостаточно средств! Чтобы пополнить баланс, обратитесь в поддержку: @suport_skup_bs_bot\n"
            "или воспользуйтесь кнопкой «Поддержка» в главном меню бота.",
            show_alert=True
        )
        await callback.message.answer(
            f"❌ <b>Недостаточно средств!</b>\n\n"
            f"Для покупки лота требуется <b>{price} ⭐</b>, а у вас на балансе <b>{balance} ⭐</b>.\n"
            "Пополните баланс через поддержку: @suport_skup_bs_bot\n"
            "или воспользуйтесь кнопкой «Поддержка» в главном меню бота.",
            parse_mode="HTML"
        )
    ...
    users_db[user.id]["balance"] -= price
    new_balance = users_db[user.id]["balance"]
    add_history_entry(user.id, -price, f"Покупка {lot['name']}")
    admin_text = (
        f"🛒 <b>ПОКУПКА ЛОТА</b>\n"
        f"👤 Пользователь: {user_data['username']} (ID: {user.id})\n"
        f"📛 Имя: {user_data['name']}\n"
        f"🎯 Купил лот: {lot['name']}\n"
        f"💰 Сумма: {price} ⭐\n"
        f"📊 Остаток: {new_balance} ⭐"
    )
    await notify_owner(user, admin_text, send_to_dm=True)
    await callback.message.answer(
        f"✅ <b>Отлично!</b>\n\n"
        f"Вы успешно купили <b>{lot['name']}</b> за {price} ⭐.\n"
        f"Продавец напишет вам в кратчайшее время.\n\n"
        f"📊 Новый баланс: {new_balance} ⭐\n"
        f"По всем вопросам обращайтесь в поддержку: @suport_skup_bs_bot\n"
        f"или воспользуйтесь кнопкой «Поддержка» в главном меню бота.",
        parse_mode="HTML"
...
async def admin_balance_amount_input(message: Message, state: FSMContext):
    ...
    if amount > 0:
        action_desc = f"ПОПОЛНИЛ БАЛАНС пользователя {users_db[target_user]['username']} (ID {target_user}) на {amount} ⭐"
    else:
        action_desc = f"СПИСАЛ БАЛАНС пользователя {users_db[target_user]['username']} (ID {target_user}) на {abs(amount)} ⭐"
    log_action(admin_user, action_desc)
    if amount > 0:
        admin_text = (
            f"💰 <b>ПОПОЛНЕНИЕ БАЛАНСА</b>\n"
            f"👤 Пользователь: {users_db[target_user]['username']} (ID: {target_user})\n"
            f"📛 Имя: {users_db[target_user]['name']}\n"
            f"➕ Сумма: +{amount} ⭐\n"
            f"📊 Новый баланс: {new_balance} ⭐"
        )
    else:
    ...
    await notify_owner(admin_user, admin_text, send_to_dm=True)
    try:
        if amount > 0:
            user_msg = f"💳 <b>Ваш баланс пополнен!</b>\n\n💰 <b>Сумма:</b> +{amount} ⭐\n📊 <b>Текущий баланс:</b> {new_balance} ⭐\n\nСпасибо, что с нами! 🎉"
        else:
            user_msg = f"💳 <b>С вашего баланса списаны средства!</b>\n\n💰 <b>Сумма:</b> {amount} ⭐\n📊 <b>Текущий баланс:</b> {new_balance} ⭐\n\nПо всем вопросам обращайтесь в поддержку: @suport_skup_bs_bot\nили воспользуйтесь кнопкой «Поддержка» в главном меню бота."
...
async def admin_freeze_user_input(message: Message, state: FSMContext):
    ...
    await message.answer(
        ...
        f"Текущий статус заморозки: {'активен' if user_data['frozen_until'] is None else 'заморожен до ' + (user_data['frozen_until'].strftime('%d.%m.%Y %H:%M') if isinstance(user_data['frozen_until'], datetime) else 'навсегда')}\n\n"
    ...
    if text == "0":
        ...
        try:
            ...
            if end_dt < get_moscow_datetime():
                await message.answer(
                    "❌ Указанная дата уже прошла! Введите будущую дату или 0 для бессрочной.",
                    reply_markup=back_to_main_kb, parse_mode="HTML"
                )
                return
            frozen_until = end_dt
            ...
            await message.answer(
                "❌ Неверный формат! Введите дату в формате <b>ДД.ММ.ГГГГ ЧЧ:ММ</b> или <b>0</b> для бессрочной.",
                reply_markup=back_to_main_kb, parse_mode="HTML"
            )
            return
    await state.update_data(freeze_until=frozen_until)
    await message.answer(
        "❄️ <b>Шаг 3: Причина заморозки</b>\n\n"
...
async def admin_freeze_reason_input(message: Message, state: FSMContext):
    ...
    await message.answer(
        f"✅ <b>Профиль пользователя {user_data['name']} заморожен!</b>\n\n"
        f"⏰ До: {until_str}\n"
        f"📌 Причина: {reason}\n\n"
        f"Уведомление отправлено пользователю.",
        reply_markup=admin_menu_kb, parse_mode="HTML"
...
async def admin_unfreeze_user_input(message: Message, state: FSMContext):
    ...
    if user_data.get("frozen_until") is None:
        await message.answer(
            "ℹ️ Профиль пользователя уже активен (не заморожен).",
            reply_markup=admin_menu_kb, parse_mode="HTML"
        )
        await state.set_state(ScamStates.admin_panel)
        return
    ...
    await message.answer(
        f"✅ <b>Профиль пользователя {user_data['name']} разморожен!</b>\n\n"
        f"Уведомление отправлено пользователю.",
        reply_markup=admin_menu_kb, parse_mode="HTML"
    )
    await state.clear()
...
async def enable_tech_break(message: Message, state: FSMContext):
    ...
    if tech_break_enabled:
        ...
        await message.answer(
            f"⚠️ Технический перерыв уже включён до **{end_str}**.\n"
            f"Если хотите изменить время, сначала отключите текущий перерыв.",
            parse_mode="HTML"
        )
        return
    await message.answer(
        "🛠️ <b>Включение технического перерыва – шаг 1</b>\n\n"
...
async def process_tech_break_time(message: Message, state: FSMContext):
    ...
    try:
        ...
        await message.answer(
            "❌ Неверный формат! Введите дату и время в формате <b>ДД.ММ.ГГГГ ЧЧ:ММ</b>.\n"
            "Например: <code>15.08.2026 20:00</code>\n\n"
            "Попробуйте ещё раз или нажмите 'Назад'.",
            reply_markup=back_to_main_kb, parse_mode="HTML"
        )
    ...
    if end_dt <= now:
        await message.answer(
            "❌ Указанное время уже прошло! Пожалуйста, введите <b>будущую</b> дату и время.\n\n"
            "Попробуйте ещё раз или нажмите 'Назад'.",
            reply_markup=back_to_main_kb, parse_mode="HTML"
        )
        return
    ...
    await message.answer(
        "🛠️ <b>Включение технического перерыва – шаг 2</b>\n\n"
...
async def process_tech_break_reason(message: Message, state: FSMContext):
    ...
    if not reason:
        await message.answer(
            "❌ Причина не может быть пустой. Введите текст причины.",
            reply_markup=back_to_main_kb, parse_mode="HTML"
        )
        return
    data = await state.get_data()
    ...
    await message.answer(
        ...
        f"🛠️ Бот будет недоступен до <b>{end_str}</b> (по московскому времени).\n"
        f"📌 <b>Причина:</b> {reason}\n\n"
        f"ℹ️ Пользователи будут видеть это сообщение.\n\n"
        f"Вы можете отключить перерыв раньше через кнопку «⛔ Отключить перерыв».",
        reply_markup=admin_menu_kb, parse_mode="HTML"
    )
    await state.set_state(ScamStates.admin_panel)
...
async def disable_tech_break(message: Message, state: FSMContext):
    ...
    if not tech_break_enabled:
        await message.answer(
            "ℹ️ Технический перерыв в данный момент не активен.",
            reply_markup=admin_menu_kb, parse_mode="HTML"
        )
        return
    tech_break_enabled = False
    ...
    tech_break_reason = ""
    await message.answer(
        "✅ <b>Технический перерыв отключён!</b>\n\n"
...
async def admin_users(message: Message, state: FSMContext):
    ...
    for idx, (uid, data) in enumerate(users_db.items(), 1):
        ...
        text += (
            f"{idx}. <b>ID:</b> <code>{uid}</code>\n"
            f"   📛 {data['name']}\n"
            f"   🔖 {data['username']}\n"
            f"   💰 Баланс: {data.get('balance', 0)} ⭐\n"
            f"   ❄️ Статус: {frozen_str}\n"
            f"   ⏰ До: {frozen_until_str}\n"
            f"   🕒 Первый визит: {data['first_seen']}\n"
            f"   🕒 Последний визит: {data['last_seen']}\n\n"
        )
        if idx >= 20:
...
async def process_new_lot_price(message: Message, state: FSMContext):
    ...
    await message.answer(
        ...
        f"💎 Гемов: {gems}\n"
        f"💰 Цена: {price} ⭐\n\n"
        f"Лот добавлен в раздел «Аккаунт».",
        reply_markup=admin_menu_kb, parse_mode="HTML"
    )
    await state.clear()
    await state.set_state(ScamStates.admin_panel)
...
async def handle_support_message(message: Message, state: FSMContext):
    ...
    try:
        ...
        await message.answer(
            f"❌ Произошла ошибка при отправке сообщения. Попробуйте позже или напишите напрямую @suport_skup_bs_bot.\n\nОшибка: {e}",
            reply_markup=support_kb, parse_mode="HTML"
        )
...
async def handle_sell(message: Message, state: FSMContext):
    await notify_owner(message.from_user, "НАЖАЛ КНОПКУ 'ПРОДАТЬ'")
    await message.answer(
        "💰 <b>Продажа аккаунта</b>\n\n"
        "👇 <b>Выберите действие:</b>",
        reply_markup=sell_menu_kb, parse_mode="HTML"
    )

@dp.message(F.text == "📦 Выставить товар")
async def handle_list_item(message: Message, state: FSMContext):
    await notify_owner(message.from_user, "НАЖАЛ КНОПКУ 'ВЫСТАВИТЬ ТОВАР'")
    await message.answer(
        "📦 <b>Выставление товара</b>\n\n"
        "📩 Чтобы выставить товар, отправьте своё объявление сюда:\n"
        "👤 <b>@suport_skup_bs_bot</b>\n"
        "или воспользуйтесь кнопкой «Поддержка» в главном меню бота.\n\n"
        "📋 <b>По шаблону:</b>\n"
        "━━━━━━━━━━━━━━━━━━━━━\n"
        ...
        "⚔️ 45 бойцов\n"
        "💰 1000 ⭐\n\n"
        "✅ После проверки модератором, ваше объявление появится в боте.",
        reply_markup=back_to_main_kb, parse_mode="HTML"
    )
...
async def main():
    ...
    print("✉️ Добавлена функция отправки сообщений пользователям")
    print("🛒 Кнопки покупки лотов и доната с проверкой баланса")
    print("🔄 Админ может вводить отрицательные числа для списания баланса")
    print("📩 Поддержка: сообщения пользователей пересылаются админу")
    print("🕒 Время первого и последнего визита исправлено (Москва UTC+3)")
    print("🛠 Обновлён техперерыв: теперь с причиной и без показа пароля")
    print("➕ Добавлена кнопка создания нового лота")
    print("❄️ Добавлена заморозка профилей с уведомлением и блокировкой функций")
    print("🔄 Добавлена кнопка разморозки профиля")
    print("⚠️ В профиле добавлено предупреждение о недоступности вывода и покупки при заморозке")
