...
async def cmd_start(message: Message, state: FSMContext):
    ...
    elif is_support(message.from_user.id):
        await message.answer("👋 Добро пожаловать, поддержка!", reply_markup=support_menu_kb)
...
# ============================================
# ===== НАПИСАТЬ ПИСЬМО (ДЛЯ ПОДДЕРЖКИ, АДМИНА, ВЛАДЕЛЬЦА) =====
# ============================================
@dp.message(F.text == "📩 Написать письмо")
async def support_write_letter(message: Message, state: FSMContext):
    # Разрешаем: поддержка, админ, владелец
    if not is_support(message.from_user.id) and not is_admin(message.from_user.id) and message.from_user.id != OWNER_USERNAME:
        await message.answer("⛔ Доступ запрещён.")
        return
    
    await message.answer(
        "✍️ <b>Написать письмо пользователю</b>\n\n"
...
async def support_msg_user_input(message: Message, state: FSMContext):
    # Разрешаем: поддержка, админ, владелец
    if not is_support(message.from_user.id) and not is_admin(message.from_user.id) and message.from_user.id != OWNER_USERNAME:
        await message.answer("⛔ Доступ запрещён.")
        return
    ...
    if target_user is None:
        await message.answer(
            ...
            "Проверьте:\n"
            "• Правильно ли вы ввели ID (только цифры)\n"
            "• Есть ли @ перед юзернеймом\n"
    ...
    await message.answer(
        f"✅ Найден пользователь: <b>{users_db[target_user]['name']}</b> ({users_db[target_user]['username']})\n\n"
        f"✍️ Теперь введите <b>текст сообщения</b>:\n\n"
...
async def support_msg_text_input(message: Message, state: FSMContext):
    # Разрешаем: поддержка, админ, владелец
    if not is_support(message.from_user.id) and not is_admin(message.from_user.id) and message.from_user.id != OWNER_USERNAME:
        await message.answer("⛔ Доступ запрещён.")
        return
    ...
    if target_user is None:
        await message.answer(
            ...
            "Начните заново через кнопку <b>«📩 Написать письмо»</b>.",
            reply_markup=main_menu_kb,
            parse_mode="HTML"
        )
        await state.clear()
    ...
    try:
        ...
        await message.answer(
            f"✅ Сообщение отправлено пользователю <b>{users_db[target_user]['name']}</b>.",
            reply_markup=support_menu_kb if is_support(message.from_user.id) else admin_menu_kb if is_admin(message.from_user.id) else owner_menu_kb,
            parse_mode="HTML"
        )
        ...
        await notify_owner(
            ...
            f"👤 Пользователю: {users_db[target_user]['name']} ({users_db[target_user]['username']})\n"
        ...
        await message.answer(
            ...
            f"Проверьте, не заблокировал ли вас пользователь.",
            reply_markup=main_menu_kb,
            parse_mode="HTML"
        )
...
# ============================================
# ===== ОБРАЩЕНИЯ (ДЛЯ ПОДДЕРЖКИ, АДМИНА, ВЛАДЕЛЬЦА) =====
# ============================================
@dp.message(F.text == "📋 Обращения")
async def support_view_appeals(message: Message, state: FSMContext):
    # Разрешаем: поддержка, админ, владелец
    if not is_support(message.from_user.id) and not is_admin(message.from_user.id) and message.from_user.id != OWNER_USERNAME:
        await message.answer("⛔ Доступ запрещён.")
        return
    ...
    if not appeals:
        await message.answer("📋 Обращений пока нет.", reply_markup=main_menu_kb)
        return
    ...
    # Определяем меню
    if message.from_user.id == OWNER_USERNAME:
        reply_menu = owner_menu_kb
    elif is_admin(message.from_user.id):
        reply_menu = admin_menu_kb
    elif is_support(message.from_user.id):
        reply_menu = support_menu_kb
    else:
        reply_menu = main_menu_kb
...
async def admin_assign_status_input(message: Message, state: FSMContext):
    ...
    for lst in [ADMIN_IDS, SUPPORT_IDS, TESTER_IDS, VIP_IDS, WORKER_IDS]:
...
async def show_main_menu(message: Message):
    ...
    elif is_support(user_id):
        await message.answer("❓ <b>Главное меню</b>", reply_markup=support_menu_kb, parse_mode="HTML")
